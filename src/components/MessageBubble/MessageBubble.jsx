...
const ImageBubble = ({ user, content, className, key }) => {
    return (
        <div key={key} className={`${className} img_bubble ${styleByUser(user, 'img_bubble-end_user', 'img_bubble-ai_coach')}`}>
            <img src={`${BASE_STORAGE_BUCKET_URL}/${content}`} className="img_bubble-img"/>
        </div>
    )
}
...
const MessageBubble = (props) => {
    ...
    return (
        <div ref={props.refToPass} className={`msg_bubble-container ${styleByUser(props.user, 'msg_bubble-container-end_user', 'msg_bubble-container-ai_coach')}`}>
            {props.user == AI_COACH ? <ProfilePic src={willowProfilePic} alt="Willow profile pic" /> : <></>}
